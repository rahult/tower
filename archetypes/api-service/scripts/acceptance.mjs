#!/usr/bin/env node
// The acceptance runner. Boots the freshly built server with a throwaway database, runs every spec
// in acceptance/specs/ against it over HTTP, and exits:
...
const dbFile = join(mkdtempSync(join(tmpdir(), "api-accept-")), "acceptance.sqlite");
...
try {
	baseUrl = await new Promise((resolve, reject) => {
		const timer = setTimeout(() => reject(new Error("the server did not report a port within 15s")), 15_000);
		...
		server.on("exit", (code) => reject(new Error(`the server exited early (${code})`)));
	});
