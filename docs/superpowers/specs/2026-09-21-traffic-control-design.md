...
- `entry_appended` events deliver each `SessionEntry` as it is written → durable transcript without polling. `message_update` is delta-only; `message_end.message` is authoritative.
