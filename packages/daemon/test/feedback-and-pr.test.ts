...
describe("review flows and the feedback gate", () => {
	...
	it("a project can turn reviews off, or choose which run", async () => {
		...
		expect((await h.api("GET", "/api/flows")).body.flows.map((flow: { name: string }) => flow.name)).toEqual(["acceptance-green", "acceptance-red", "adversarial-review", "deep-research", "invariant-diff", "invariant-simulation", "plan-coach", "solid-review", "understand-system"]);
	});
});
