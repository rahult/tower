...
import { bootHarness, buildingTurn, type Harness, planningTurn } from "./harness.ts";

let h: Harness;
afterEach(async () => h?.close());

/** Scripts each session by its stage, so re-plans and builds get the right behaviour. */
const byStage = () => (spec: { sessionId: string }) => (spec.sessionId.includes("-plan-") ? [planningTurn()] : [buildingTurn()]);
...
describe("plan gate and building handoff", () => {
	...
	it("approval builds in a fresh session that sees the plan path but none of the planning conversation", async () => {
		...
		const [planner, builder] = h.driver.handles;
	...
	it("refuses to decide a gate twice or to decide another card's gate", async () => {
		...
		expect(h.driver.handles).toHaveLength(2);
	});
	...
	it("a build that reports failure asks for attention, and retry re-runs it with guidance", async () => {
		...
		h = await bootHarness((spec) => {
			...
			return ++builds === 1 ? [{ events: [], effect: async () => void (await import("node:fs")).writeFileSync(`${spec.sessionDir}/../stage-result.json`, '{"status":"fail","summary":"Tests are red."}') }] : [buildingTurn()];
		});
		const { card, gate } = await planCard(h);
		await h.api("POST", `/api/cards/${card.id}/gates/${gate.id}`, { decision: "approve" });
		await h.daemon.whenIdle();
		...
		expect((await h.api("GET", `/api/cards/${card.id}`)).body.card).toMatchObject({ stage: "building", status: "idle", needsAttentionReason: null });
	});
});
