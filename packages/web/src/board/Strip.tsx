...
export function Strip({ card, selected, onOpen, action }: StripProps) {
	...
	return (
		<li className={`flex items-stretch rounded-[3px] text-ink ${TINT_CLASS[tint]} ${selected ? "ring-2 ring-chalk ring-offset-2 ring-offset-rack" : ""}`}>
			...
			<button type="button" onClick={onOpen} className="grid min-w-0 flex-1 cursor-pointer grid-cols-[5.75rem_1fr] text-left sm:grid-cols-[5.75rem_1fr_7rem_10rem]">
				...
				<span className="condensed hidden border-l border-ink/25 px-2 py-2 text-[14px] sm:block">{stage}</span>
				<span className="condensed hidden border-l border-ink/25 px-2 py-2 text-[14px] font-semibold sm:block">{status}</span>
			</button>
